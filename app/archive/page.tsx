...
import {
    Archive, ChevronLeft, RotateCcw, Trash2,
    Loader2, Search, Calendar, PackageX
} from 'lucide-react';
...
export default function ArchivePage() {
    ...
    const handleEmptyArchive = async () => {
        if (!confirm('Are you sure you want to delete all archived notes? This action cannot be undone.')) return;
        setLoading(true);
        try {
            ...
            toast.error('Failed to empty archive completely');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1">Restore or permanently delete archived notes</p>
                        </div>
                    </div>
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20 space-y-4">
                        ...
                        <p className="text-gray-500">Loading archive...</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {filteredNotes.map(note => (
                            <div
                                key={note.id}
                                className="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm flex items-center justify-between group transition-all hover:border-amber-200 dark:hover:border-amber-900"
                            >
                                <div className="space-y-1">
                                    ...
                                    <div className="flex items-center gap-4 text-xs text-gray-400">
                                        <div className="flex items-center gap-1">
                                            ...
                                            Archived {(note as any).archived_at ? new Date((note as any).archived_at).toLocaleDateString() : 'Unknown'}
                                        </div>
                                        {note.word_count && (
                                ...
                                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    >
                                        {isActioning === note.id ? <Loader2 className="h-5 w-5 animate-spin" /> : <Trash2 className="h-5 w-5" />}
                                    </button>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                        ...
                        <div className="space-y-1">
                            <h3 className="font-bold text-xl">Archive is empty</h3>
